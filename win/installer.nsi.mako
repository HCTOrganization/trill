...
Section
    <%text>$</%text>{IfNot} <%text>$</%text>{AtLeastWin8}
        MessageBox MB_OK|MB_ICONSTOP "$(MessageNoNT61Support)"
        Quit
    <%text>$</%text>{EndIf}
    ...
    WriteRegStr HKCU "<%text>$</%text>{REGPATH_UNINSTSUBKEY}" "DisplayVersion" "5.0.0.0"
