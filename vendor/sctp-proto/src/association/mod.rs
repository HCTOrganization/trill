...
/// TANGO PATCH: congestion-window floor, in bytes. RFC 4960 collapses cwnd to
/// 1–4 MTU on loss (fast retransmit halves it; a T3 timeout drops it to 1*MTU),
/// which throttles a tiny real-time data channel to cwnd/RTT — under sustained
/// loss that starves it well below the ~60 Hz it needs, the redundancy window
/// and RTT bloat, and inputs stop arriving in time (both peers overflow their
/// unconfirmed-input queue and the match bails). Tango's data rate is a few
/// KB/s, so there is no real congestion to control; flooring cwnd high keeps
/// SCTP from throttling it while leaving the rest of the algorithm intact. 256
/// KiB covers both the per-frame in-match stream and the pre-match save-state
/// burst with headroom even at multi-second RTT.
const MIN_CWND: u32 = 256 * 1024;
...
impl Association {
    fn new_common(
        ...
        let cwnd = (4 * mtu).min((2 * mtu).max(4380)).max(MIN_CWND); // TANGO PATCH: floor
                ...
                if let Some(c) = self.inflight_queue.get_mut(tsn) {
                    if !c.acked && !c.abandoned() && c.miss_indicator < 3 {
                        ...
                        if c.miss_indicator == 3 && !self.in_fast_recovery {
                            ...
                            self.cwnd = self.ssthresh.max(MIN_CWND); // TANGO PATCH: floor
    ...
    fn on_retransmission_timeout(&mut self, timer_id: Timer, n_rtos: usize) {
        match timer_id {
            ...
            Timer::T3RTX => {
                ...
                self.ssthresh = core::cmp::max(self.cwnd / 2, 4 * self.mtu);
                self.cwnd = self.mtu.max(MIN_CWND); // TANGO PATCH: floor
                trace!(
