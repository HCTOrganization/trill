...
settings-data-folder = 데이터 폴더
settings-data-folder-change = 변경…
settings-streamer-mode = 스트림 개인 정보 보호 모드
settings-section-experimental = 실험적 기능
settings-enable-save-editor = 세이브 에디터 활성화
settings-experimental-warning = 메인터너는 매치 메이킹 엔드포인트에서 사용되었던 서버측 소스 코드를 삭제하고 AGPL을 위반했다. 따라서 이 서버를 더 이상 사용하지 않는 것이 좋습니다.
...
settings-theme-color = 테마 색상
settings-theme-color-trill-yellow = 트릴 옐로우
settings-theme-color-pegasus-blue = 페가수스 블루
settings-theme-color-sonia-pink = 미소라 핑크
settings-theme-color-zerker-grey = 베르세르크 그레이
settings-theme-color-ninja-green = 시노비 그린
settings-theme-color-saurian-orange = 다이나소 오렌지
...
settings-netplay-endpoint-tango-warning = 관리자가 이 서버에서 사용하는 신호 전송 서버 소스 코드를 제거하여 AGPL을 위반했습니다. 따라서 이 서버의 사용은 더 이상 권장되지 않습니다.
...
settings-border-preference = 에뮬레이터 경계 환경설정
settings-border-preference-bnlc = 어드밴스드 컬렉션
settings-border-preference-custom = 사용자 지정
settings-border-preference-locate = 찾기
settings-border-preference-disable = 경계 표시 안 함
