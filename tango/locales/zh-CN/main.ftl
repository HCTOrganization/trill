...
settings-theme-color-speaki-brown = 斯皮奇棕
...
settings-netplay-endpoint-easserver = Trill (亚服)
settings-netplay-endpoint-tango = Tango
settings-netplay-endpoint-tango-warning = 维护者删除了该配对服务器端点使用的服务端源代码，违反了AGPL。因此，不再推荐使用此服务器。
...
settings-border-preference = 模拟器边框偏好
settings-border-preference-bnlc = EXE合集
settings-border-preference-custom = 自定义
settings-border-preference-locate = 选择背景
settings-border-preference-disable = 不显示边框
save-tab-cover = 封面
save-tab-navi = 领航员
save-tab-folder = 芯片夹
...
settings-audio-enable-startup-voice = 启用启动语音
settings-audio-enable-prebattle-voice = 启用对战前语音
settings-audio-prebattle-voice-volume = 语音音量
